...
// Creates a copy of key value pair
...
// Free's the current value ( because we allocate memory for it )
// then creates new memory depending on the length of the new value
...
// This Struct Creates a Node Which has the following:
// -	A Pointer to a ( key , value ) pair block
// -	A Pointer to a Node( next ) this links the nodes together
struct kvlist_node_t {
  kvpair_t *kv;
  struct kvlist_node_t *next;
};

// Creates a new Node or block to store our key, value pair
...
// Free's Node Block and key value pair
...
// The Singly linked list pointing to the head and tail
// of our Node Block struct
struct kvlist_t {
  kvlist_node_t *head;
  kvlist_node_t *tail;
};
...
// Appends a key value pair instance to the list
...
// Concentates other to list ( list + other = list )
